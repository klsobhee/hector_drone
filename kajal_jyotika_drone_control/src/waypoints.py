...
import numpy as np
from kajal_jyotika_drone_control.srv import *
import rospy
import rosservice
...
if __name__ == "__main__":
    ...
    rospy.wait_for_service('drone_move_service') 
    while not rospy.is_shutdown():
        if(flag==0):
            ...
            for i in range(len(waypoint_x)):
                ...
                drone_move_service = rospy.ServiceProxy('drone_move_service', position_control) 
            ...
            cmd.linear.z  = 0.0
            pub_cmd_vel.publish(cmd)
            print('Hector Robot mission commpleted sucessfully. All Waypoints covered.')
            rate.sleep()
